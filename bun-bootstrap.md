...
### P1: Remove watcher synthetic duplicate event workaround (2026-02-26)

- Removed the FreeBSD synthetic duplicate event emission in `src/bun.js/node/path_watcher.zig` directory-rescan fallback
- Previous cleanup attempt (earlier in Phase D) regressed `fs.promises.watch > add file/folder to folder` (timeout)
- Root cause of that regression was the `fs.promises.watch` async iterator lost-wakeup race, which was subsequently fixed
- With the lost-wakeup fix in place, single events from the rescan fallback are sufficient — no duplicate needed
- The directory rescan fallback itself remains (`keep`) — required because FreeBSD kqueue doesn't provide child-name payloads
- Validation: full Phase E core gate green
  - `fs.watch.test.ts` => `32 pass / 0 fail` (5 consecutive runs)
