...
5. Review and downscope/remove FreeBSD-specific debug tracing hooks before upstreaming.
   - Currently used for diagnosis during Phase C/D/E:
     - `BUN_FREEBSD_SPAWN_TRACE` — removed (was in `process.zig`, `subprocess.zig`, `subproc.zig`, `js_bun_spawn_bindings.zig`)
     - `BUN_FREEBSD_FILESINK_TRACE` — removed (was in `FileSink.zig`)
     - `BUN_FREEBSD_MODULE_TRACE` — removed (was in `ZigGlobalObject.cpp`)
   - Desired outcome:
...
| watcher directory rescan fallback | `src/bun.js/node/path_watcher.zig` | `keep` | FreeBSD kqueue lacks child-name payloads for directory notifications, so rescan fallback is required (`keep`). Synthetic duplicate event workaround removed — was only needed before `fs.promises.watch` async iterator lost-wakeup fix. | `test/js/node/watch/fs.watch.test.ts` (`32 pass / 0 fail` x5) |
| `rmdir` errno normalization | `src/js/node/fs.ts`, `src/js/node/fs.promises.ts`, `src/bun.js/node/node_fs.zig` | `resolved` | Fully cleaned up: JS shims removed (redundant after `freebsd_errno.zig` landed); Zig-layer manual errno-66 interception also removed (redundant now that `freebsd_errno.zig` maps `ENOTEMPTY = 66` and `errnoSysP` handles it natively). | `test/js/node/fs/fs.test.ts` (`234 pass / 6 skip / 0 fail`) |
...
| `src/bun.js/api/bun/subprocess.zig` | (FreeBSD spawn trace hooks removed) | `resolved` | All `BUN_FREEBSD_SPAWN_TRACE` debug instrumentation removed from `process.zig`, `subprocess.zig`, `subproc.zig`, and `js_bun_spawn_bindings.zig`. | Full Phase E gate green after removal. |
| `src/bun.js/webcore/FileSink.zig` | (FreeBSD completion-order workaround and trace hooks removed) | `resolved` | FreeBSD-specific `this.end(null)` path in `handleResolveStream()` removed; now uses `this.writer.close()` like all platforms. `BUN_FREEBSD_FILESINK_TRACE` debug hooks removed. Root cause was in `ProcessObjectInternals.ts` (P0-5 fix). | `spawn-stdin-readable-stream.test.ts` (20/0 x5), full Phase E gate green. |
...
3. Debug tracing helpers (`BUN_FREEBSD_SPAWN_TRACE`, `BUN_FREEBSD_FILESINK_TRACE`, `BUN_FREEBSD_MODULE_TRACE`) have been removed; equivalent diagnostics available through generic `Output.scoped` logging if needed in future.
...
| `src/bun.js/node/path_watcher.zig` | FreeBSD kqueue directory rescan fallback when event payload lacks names | `keep` | Directory rescan fallback is required because FreeBSD kqueue directory notifications do not provide child-name/op granularity. Synthetic duplicate event workaround removed — was only needed before `fs.promises.watch` async iterator lost-wakeup fix. | `test/js/node/watch/fs.watch.test.ts` (`32 pass / 0 fail` x5) |
| `src/bun.js/node/node_fs.zig` | FreeBSD copy/cp read-write fallback paths | `keep` | FreeBSD copy/cp read-write fallback is runtime platform support. Previous `rmdir` ENOTEMPTY normalization and Zig 0.13 readFile workarounds have all been removed (redundant after `freebsd_errno.zig` and compiler baseline updates). | `test/js/node/fs/fs.test.ts` (`234 pass / 6 skip / 0 fail`); targeted `mkdtemp`, `cp` repros logged in `bun-bootstrap.md` |
| `src/js/node/fs.ts` | (rmdir normalization removed) | `resolved` | JS `EREMOTE -> ENOTEMPTY` shim removed; lower-layer `freebsd_errno.zig` now handles this natively. | `test/js/node/fs/fs.test.ts` (`rmdir` cases) |
| `src/js/node/fs.promises.ts` | (rmdir normalization removed) | `resolved` | Same as `fs.ts`; JS shim removed after confirming `freebsd_errno.zig` maps `ENOTEMPTY = 66` correctly. | `test/js/node/fs/fs.test.ts` (`promises.rmdir` cases) |
...
1. The highest-risk temporary shim remaining in this cluster is:
   - `path_watcher.zig` synthetic duplicate event emission — removed (was only needed before `fs.promises.watch` lost-wakeup fix)
2. Previously tracked shims in this cluster have been resolved:
   - JS-level `rmdir` errno normalization in `src/js/node/fs.ts` and `src/js/node/fs.promises.ts` (removed; redundant after `freebsd_errno.zig`)
   - `node_fs.zig` Zig-layer `rmdir` errno-66 interception (removed; redundant after `freebsd_errno.zig`)
   - `node_fs.zig` Zig 0.13/FreeBSD release-build readFile/readFileSync workarounds (removed; no longer needed on current compiler baseline)
3. Watcher and copy/sendfile backend support itself is now clearly classified as `keep` and should not be conflated with the temporary compatibility layers above it.
4. `node_fs.zig` FreeBSD-specific behavior is now reduced to copy/cp read-write fallback paths only (`keep`).
...
| `src/bun.js/bindings/ZigGlobalObject.cpp` | (FreeBSD module trace hooks removed) | `resolved` | All `BUN_FREEBSD_MODULE_TRACE` debug instrumentation removed (function definitions + 17 call sites in resolve/import/fetch/evaluate paths). | Full Phase E gate green after removal. |
...
| P1 | `FileSink` FreeBSD completion-order workaround (`stream.done()` defer) | `src/bun.js/webcore/FileSink.zig` | `resolved` | Fully resolved: FreeBSD-specific `this.end(null)` path in `handleResolveStream` removed (now uses `this.writer.close()` like all platforms). Debug trace hooks (`BUN_FREEBSD_FILESINK_TRACE`) also removed. Root cause was in `ProcessObjectInternals.ts` (P0-5); FileSink workaround was masking same issue. | N/A (resolved). | Full Phase E gate green; `spawn-stdin-readable-stream` 5/5 reliable after removal. |
| P1 | watcher synthetic duplicate event workaround | `src/bun.js/node/path_watcher.zig` | `resolved` | Fully resolved: synthetic duplicate event removed from FreeBSD directory-rescan fallback. Previous cleanup attempt regressed `fs.promises.watch` timeout, but that was before the `fs.promises.watch` async iterator lost-wakeup race fix was committed. With that fix in place, single events are sufficient. The rescan fallback itself (`keep`) remains — it's required because FreeBSD kqueue directory notifications lack child-name payloads. | N/A (resolved). | `fs.watch.test.ts` (32/0 x5), full Phase E gate green. |
...
| P1 | `node_fs.zig` FreeBSD + Zig 0.13 readFile* compiler workarounds | `src/bun.js/node/node_fs.zig` | `resolved` | Fully resolved: small-file pre-stat fast-path disable, `result_bytes` duplicate path, string-return union special-case, and explicit len-assignment special-case were all removed and validated on current compiler baseline. | N/A (resolved). | `fs.test.ts` + targeted small-file `readFileSync` string paths pass after each reduction step. |
...
| P3 | FreeBSD-specific debug trace hooks | `src/bun.js/api/bun/process.zig`, `src/bun.js/api/bun/subprocess.zig`, `src/shell/subproc.zig`, `src/bun.js/api/bun/js_bun_spawn_bindings.zig`, `src/bun.js/webcore/FileSink.zig`, `src/bun.js/bindings/ZigGlobalObject.cpp` | `resolved` | Fully resolved: all `BUN_FREEBSD_SPAWN_TRACE`, `BUN_FREEBSD_FILESINK_TRACE`, and `BUN_FREEBSD_MODULE_TRACE` env-gated debug trace hooks removed from all 5 Zig files and 1 C++ file. No runtime behavior change (traces were debug-only no-ops in production). | N/A (resolved). | Full Phase E gate green; spawn-stdin 5/5 reliable after removal. |
